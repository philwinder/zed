...
impl DisplaySnapshot {
    ...
    pub fn inlay_hint_at(&self, point: DisplayPoint) -> Option<(&Inlay, usize)> {
        let wrap_indent = point
            .row()
            .0
            .checked_sub(1)
            .and_then(|previous_row| self.soft_wrap_indent(DisplayRow(previous_row)));
        if wrap_indent.is_some_and(|indent| point.column() < indent) {
            return None;
        }
        self.inlay_snapshot()
            .inlay_hint_at_offset(self.display_point_to_inlay_offset(point, Bias::Left))
    }
